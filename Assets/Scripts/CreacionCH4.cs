using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CreacionCH4 : MonoBehaviour
{
    public GameObject targetC;
    public GameObject targetH;
    public GameObject targetH2;
    public GameObject targetH3;   
    public GameObject targetH4;  
    private bool detecC;
    private bool detecH;
    private bool detecH2;
    private bool detecH3;
    private bool detecH4;
    public GameObject prefab;
    private GameObject molecula;
    private Vector3 distancia;
    private Vector3 distancia2;
    private Vector3 distancia3;
    private Vector3 distancia4;
    public bool creado;
    public int umbral;
    private GameObject texto;   //---------------------------------
    private bool infoAct;       //---------------------------------
    
    // Start is called before the first frame update
    void Start()
    {
        creado =  false;
        detecC = false;
        detecH= false;
        detecH2= false;
        detecH3= false;
        detecH4= false;
        infoAct = false;  //---------------------------------
    }

    // Update is called once per frame
    void Update()
    {
        if (detecH && detecC && detecH2 && detecH3 && detecH4){
            
        distancia = targetC.transform.position - targetH.transform.position;
        distancia2 = targetC.transform.position - targetH2.transform.position;
        distancia3 = targetC.transform.position - targetH3.transform.position;
        distancia4 = targetC.transform.position - targetH4.transform.position;

        if(distancia.magnitude < umbral && distancia2.magnitude < umbral && distancia3.magnitude < umbral && distancia3.magnitude < umbral && creado == false){

            crearMolecula();
        }
        }

        if(creado == true && condicionesDestruccion())
        {
            destruirMolecula();
        }

         if(Input.GetKeyDown(KeyCode.I)){             //------------------------------------------------
            if (!infoAct) infoAct = true;
            else infoAct = false;
        }
        if(creado){
               if(infoAct) texto.SetActive(true);
               else texto.SetActive(false);
        }                                            //------------------------------------------------



    }

    public bool condicionesDestruccion(){
        bool dest = false;

        if (distancia.magnitude > umbral || distancia2.magnitude > umbral || distancia3.magnitude > umbral ||  distancia4.magnitude > umbral) dest = true;
        else if(!detecH || !detecC || !detecH2 || !detecH3 || !detecH4) dest = true;

        return dest;
        
    }

    public void crearMolecula(){
        
        desactivarHijos(targetC);
        desactivarHijos(targetH);
        desactivarHijos(targetH2);
        desactivarHijos(targetH3);
        desactivarHijos(targetH4);
            creado = true;
             molecula = Instantiate(prefab, targetC.transform.position,transform.rotation);
              texto = molecula.transform.GetChild(4).gameObject;   //------------------------------------------------------------
             texto.SetActive(false);                                //---------------------------------
    }
    
    public void destruirMolecula(){
        creado = false;
        Destroy(molecula);
        activarHijos(targetC);
        activarHijos(targetH);
        activarHijos(targetH2);
        activarHijos(targetH3);
        activarHijos(targetH4);
    }

      public void desactivarHijos(GameObject padre)
    {
        foreach (Transform hijo in padre.transform)
        {
            hijo.gameObject.SetActive(false);
        }
    }

     public void activarHijos(GameObject padre)
    {
        foreach (Transform hijo in padre.transform)
        {
            hijo.gameObject.SetActive(true);
        }
    }

    public  void noDetectarC(){
         detecC = false;
        
    }
    public void noDetectarH(){
        detecH = false;
    }
    public void noDetectarH2(){
        detecH2 = false;
    }
     public void noDetectarH3(){
        detecH3 = false;
    }
    public void noDetectarH4(){
        detecH4 = false;
    }
    public  void detectarC(){
         detecC = true;
    }
    public void detectarH(){
        detecH = true;
    }
    public void detectarH2(){
        detecH2 = true;
    }
    public void detectarH3(){
        detecH3 = true;
    }
     public void detectarH4(){
        detecH4 = true;
    }

    }

